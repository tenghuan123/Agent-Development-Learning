...
```text
[x] V0  LLM Chat         ── LLM 原生机制 / Token / Context Window / 记忆本质与结构化输出 (当前阶段)
[ ] V1  Tool Calling     ── 模型行动力 / Tool Schema 契约与本地 Runtime 执行器
[ ] V2  Agent Loop       ── Thought → Action → Observation 闭环与死循环熔断
...
1. **实验 1：模型记忆本质 (Statelessness Lab)**
   - 探究为什么 LLM 原生没有状态，所谓的“记忆”完全是 Runtime 每次将历史消息重新喂入 Context Window 的结果。
2. **实验 2：Prompt 约束 vs Structured Schema (Schema Lab)**
