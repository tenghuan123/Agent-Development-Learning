# Mini Claude Code 课程主线与演进全景

学习 Agent 最佳的方式：**不是先学 LangChain API，而是先遇到一个真实工程问题，再为了破解这个问题引入一个新概念。**

把整个学习过程设计成一个项目逐渐“长大”的过程。毕业项目就是一个 **Mini Claude Code**。每一课都遵循同一个结构：
...
> **结论：此时系统仍然只是一个纯文本交互的 Chatbot。**

---

## 第二阶段：模型为什么只会“说”，不会“做”？

当我们尝试让它辅助真实编程时，立即遇到了第一个瓶颈。
...
> **核心认知：Tool 设计其实是在给 AI 设计交互界面（Tool Engineering）。**
...
> **核心机制：经典的 Agent Loop（Thought → Action → Observation 闭环）。**
...
> **核心突破：Agent Engineering（循环控制、状态自愈与熔断机制）。**
...
> **核心突破：真正的 Context Engineering（上下文预算与信息治理）。**
...
### 引入方案：Harness Engineering（控制与防护机制）
...
> **核心认知：这已经不仅仅是 AI 算法问题，而是严肃的系统工程。**
...
### 引入方案：MCP (Model Context Protocol)
...
### 引入方案：Durable Execution 与状态机图（如 LangGraph 原理）
...
### 引入方案：Evaluation 与 Tracing 评测体系
...
> **核心突破：Agent Engineering 第一次变成真正可量化的严肃工程。**
...
这就是这套演进体系希望带给开发者的深层认知。

---

## 演进阶梯全景表

把这 11 个阶段连缀成清晰可执行的演进路径：
...
> **这正是一门《从 0 手写 Mini Claude Code》的完整探索之旅。**
...
从第一课最简朴的无状态 CLI，到第二课 Tool Calling、第三课 Agent Loop、第四课自愈修复代码……直到构建出具备权限沙箱、标准协议、容灾检查点与严肃评测体系的生产级系统。

每一课都不会直接抛出既定答案，而是先抛出最真实的工程痛点：

> **“一个 AI 聊天程序最少需要哪些东西？模型到底有没有记忆？”**  
> **“为什么 AI 只会告诉你怎么干，却不能真的干？”**

带着问题设计实验、推导方案、手写代码，并用明确标准验收。一路踩坑、一路生长，真正将几十行原型代码，打磨成一个功能完备的 Mini Claude Code。
